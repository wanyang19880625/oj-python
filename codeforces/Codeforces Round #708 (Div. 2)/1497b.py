...
# @url: https://codeforc.es/contest/1497/problem/B
import sys, os
...
def main():
	...
	for i in range(t):
		...
		for j in range(1, (m + 1) // 2):
			if j in d.keys() and m - j not in d.keys():
				ans += d[j]
			if j not in d.keys() and m - j in d.keys():
				ans += d[m - j]
			if j in d.keys() and m - j in d.keys():
				ans += 1
				x = d[j] + d[m - j]
				if abs(d[j] - d[m - j]) > 1:
					ans += x - 2 * min(d[j], d[m - j]) - 1
		print(ans)
