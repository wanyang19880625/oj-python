...
def main():
	...
	for i in range(t):
		...
		for j in range(1, n):
			tmp = pos[j]
			if tmp[0] > first[2] or tmp[2] < first[0]:
				f = False
				break
			if tmp[3] > first[1] or tmp[1] < first[3]:
				f = False
				break
			first[0], first[2] = max(tmp[0], first[0]), min(tmp[2], first[2])
			first[1], first[3] = min(first[1], tmp[1]), max(first[3], tmp[3])
		if f:
			print(1, first[0], first[1])
		else:
			print(0)
