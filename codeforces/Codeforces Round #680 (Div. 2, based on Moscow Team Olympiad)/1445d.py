...
def main():
    ...
    sa=sorted(a)
    ## combination (组合数规律),2n个数分为两组,分组总数C(2n,n)
    ## split array to two,every f(p-q) = the sum of n big numbers - the sum of n small numbers
    mod=998244353
    # 快速幂
    ...
    cnt=fact[2*n]*infact[n]%mod*infact[n]%mod
    ans=cnt*(sum([sa[i] for i in range(n,2*n)])-sum([sa[i] for i in range(n)]))
    print (ans%mod)
